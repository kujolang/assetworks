...
Backup preserves exact immutable record/event/journal bytes in a sharded layout. Referenced media is copied into `artifacts/SHA256.blob`, deduplicated by digest, with a fixed 64 KiB native buffer and a 4 GiB per-artifact ceiling. Inventory pages contain at most 1,000 entries and 1 MiB. Only managed evidence and referenced media are copied; arbitrary files, keys, transient adapter files and unrelated directories are excluded. The completion manifest is written last, after independent validation of the copied state and media.
